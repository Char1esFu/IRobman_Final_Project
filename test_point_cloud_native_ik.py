...
def generate_cartesian_trajectory(sim, ik_solver, start_joints, target_pos, target_orn, steps=100):
    ...
    for step in range(steps + 1):
        ...
        # solve IK for current Cartesian position using PyBullet
        current_joints = solve_ik_pybullet(
            sim.robot.id,
            sim.robot.ee_idx,
            pos,
            target_orn,
...
def solve_ik_pybullet(robot_id, ee_idx, target_pos, target_orn, joint_indices):
    ...
    """
    # print(f"Robot ID: {robot_id}, End-effector index: {ee_idx}, Target position: {target_pos}, Target orientation: {target_orn}, Joint indices: {joint_indices}")
...
def run(config):
    ...
    for viewpoint_idx, (target_pos, target_orn) in enumerate(zip(target_positions, target_orientations)):
        ...
        # Use PyBullet IK solver instead of DifferentialIKSolver
        target_joints = solve_ik_pybullet(
            sim.robot.id, 
            sim.robot.ee_idx, 
            target_pos, 
            target_orn, 
