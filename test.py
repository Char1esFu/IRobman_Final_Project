...
def run(config):
    ...
    target_positions = [
        np.array([-0.2, -0.45, 1.7]),  # First viewpoint
        np.array([0.2, -0.45, 1.7])    # Second viewpoint
    ]
    target_orientations = [
        p.getQuaternionFromEuler([0, np.radians(135), 0]),  # First viewpoint
        p.getQuaternionFromEuler([0, np.radians(-135), 0])  # Second viewpoint
    ]
