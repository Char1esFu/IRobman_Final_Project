...
def refine_registration(source, target, voxel_size):
    ...
    Returns:
    result: refined registration result
    """
    print(":: Point-to-plane ICP registration is applied on original point")
    print("   clouds to refine the alignment. This may take a few minutes.")
...
def merge_point_clouds(collected_data, voxel_size=0.005):
    """
    ...
    """
    if not collected_data or len(collected_data) < 2:
        print("Need at least 2 point clouds to merge")
        return None
    
    print("Starting point cloud merging process...")
    
    # Get the first point cloud as target
    target_pcd = collected_data[0]['point_cloud']
    print(f"Using first point cloud as target with {len(target_pcd.points)} points")
    ...
    # Register and merge each subsequent point cloud
    for i in range(1, len(collected_data)):
        source_pcd = collected_data[i]['point_cloud']
        print(f"\nProcessing point cloud {i+1} with {len(source_pcd.points)} points")
        
        # Preprocess source point cloud
        ...
        if result_ransac.fitness < 0.3:
            print("Global registration failed, skipping this point cloud")
            continue
        
        print(f"Global registration fitness: {result_ransac.fitness}")
        
        # Refine registration
        result_icp = refine_registration(source_pcd, target_pcd, voxel_size)
        ...
        # Merge point clouds
        merged_pcd = merged_pcd + source_pcd
        
        # Update target for next iteration
        target_pcd = merged_pcd
        target_down, target_fpfh = preprocess_point_cloud(target_pcd, voxel_size)
    
    # Final processing
    print("\nPerforming final processing on merged point cloud...")
...
if __name__ == "__main__":
    ...
    # Visualize the individual point clouds
    if collected_point_clouds:
        ...
        print("\nMerging point clouds using ICP...")
        merged_pcd = merge_point_clouds(collected_point_clouds)
        
        if merged_pcd is not None:
            print("\nVisualizing merged point cloud...")
            # Create a list with the merged point cloud for visualization
            merged_data = [{'point_cloud': merged_pcd}]
            visualize_point_clouds(merged_data)
            
            # Save the merged point cloud
            output_path = "merged_point_cloud.ply"
